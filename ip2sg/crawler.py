...
session = RequestUtil(main_page_url=main_page_url, timeout=120, sleep_seconds=10, retry_times=3, headers=headers, default_headers_enable=False, os=OS.MACOS, browser=Browser.FIREFOX.value)
...
class Crawler(BaseCrawler):
    ...
    def collect_items(self, input, page):
        if page == 1:
            page = "ctl00$PlaceHolderMain$uclSimpleSearch$PagerIP$ddlTopSelect"
        elif page == -1:
            page = "ctl00$PlaceHolderMain$uclSimpleSearch$PagerIP$lnkLast"
        else:
            page = "ctl00$PlaceHolderMain$uclSimpleSearch$PagerIP$lnkPage{}".format(page)
        ...
        body['ctl00$PlaceHolderMain$uclSimpleSearch$txtSearchText'] = input
        body['__EVENTTARGET'] = page
        response = self.session.post(url=main_page_url, body=body)
        results = []
        items = []
        total_page = 0
        ...
        try:
            ...
            results = odds + evens
            print(len(results))
            # Page 1 / 1 of 1 record(s)
            # No Record Found
            total_page = document.select_one('div#ctl00_PlaceHolderMain_uclSimpleSearch_PagerIP_lblPage')
            if total_page:
            ...
            logger.error('Fail to parse input %s', input)
    ...
    def start_crawler(self, input):
        ...
        try:
            ...
            self.save()
            logger.info('Total Got %s items of %s', total_count, input)
